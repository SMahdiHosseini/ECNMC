...
def read_data_flowIndicator(__ns3_path, rate):
    flows_name = []
    file_paths = glob.glob('{}/scratch/Results_forward/{}/0/*_EndToEnd.csv'.format(__ns3_path, rate))
...
for rate in serviceRateScales:
    ...
    for file in os.listdir('../results_postProcessing_forward/'+str(rate)+'/'):
        temp = {}
        if file.startswith(check):
            with open('../results_postProcessing_forward/'+str(rate)+'/'+file) as f:
                temp = js.load(f)
...
ax2.set_xticklabels(droprates_xticks)
ax2.set_xlabel('Drop Rate')
# plt.grid(True)
plt.title('Success Rate for Different Drop Rates')
plt.savefig('../results_postProcessing_forward/{}_success_perDropRate.png'.format(check))
plt.clf()

print(utilizationFactors)

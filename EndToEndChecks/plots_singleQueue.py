...
def plot_success_per_rate(results, flows, paths, rates, results_dir):
    for metric in set(k for r in results.values() for k in r.keys()):
        ...
        plt.savefig(f"../Results/results_{results_dir}/{metric}_vs_Rate.png")
        plt.close()
...
def __main__():
    ...
    plot_success_per_rate(results, flows, paths, serviceRateScales, results_dir)
__main__()

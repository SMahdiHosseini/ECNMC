        ...
        TypeId("ns3::TcpSocketBase")
            ...
            .AddAttribute("UseEcn",
                          ...
                          MakeEnumChecker(TcpSocketState::Off,
                                          ...
                                          "AcceptOnly"))
            .AddAttribute("ProbeClockInterval", // mahdi start
                          "Interval for the probe clock",
                            TimeValue(MilliSeconds(0)),
                            MakeTimeAccessor(&TcpSocketBase::SetProbeClockInterval, &TcpSocketBase::GetProbeClockInterval),
                            MakeTimeChecker()) // mahdi end
            .AddTraceSource("RTO",
...
// mahdi start
void TcpSocketBase::StartMeasurementProbeClock()
{
    NS_LOG_FUNCTION(this);
    if (m_probeClockInterval.IsStrictlyPositive())
    {
        Simulator::Schedule(m_probeClockInterval, &TcpSocketBase::MeasurementProbeClockTick, this);
    }
}

void TcpSocketBase::MeasurementProbeClockTick()
{
    NS_LOG_FUNCTION(this);
    if (m_probeClockInterval.IsStrictlyPositive())
    {
    ...
    // std::cout << "Probe clock measurement at time: " << Simulator::Now().GetSeconds() << " seconds" << std::endl;
    Time nextSendingTime = Time::Max();
    if (m_txBuffer->Size() > 0)
    {
        if (IsPacingEnabled())
        {
            if (m_pacingTimer.IsExpired())
            {
                ...
                return; // TCP will send something soon
            }
            else
            {   
                // std::cout << "Pacing timer is not expired, checking delay left." << std::endl;
                nextSendingTime = Min(m_pacingTimer.GetDelayLeft(), nextSendingTime); 
            }
        }
        else
        {
            // std::cout << "Pacing is not enabled" << std::endl;
            return; // we hope to send something soon
        }
    }

    if (nextSendingTime > m_probeClockInterval)
    {
        // std::cout << "Next sending time is too far in the future." << std::endl;
        SendEmptyPacket(TcpHeader::NONE); // Send an empty packet to probe the clock
    }
}

void TcpSocketBase::SetProbeClockInterval(Time interval)
{
    NS_LOG_FUNCTION(this << interval);
    m_probeClockInterval = interval;
}

Time TcpSocketBase::GetProbeClockInterval() const
{
    NS_LOG_FUNCTION(this);

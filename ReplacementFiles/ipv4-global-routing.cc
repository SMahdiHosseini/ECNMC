...
// ****** Mahdi Change ***** (START) ***** // 
uint64_t
Ipv4GlobalRouting::GetHashValue(const Ipv4Header header, Ptr<const Packet> ipPayload)
{
...
Ptr<Ipv4Route>
Ipv4GlobalRouting::LookupGlobal(Ipv4Address dest, const Ipv4Header& header, Ptr<const Packet> ipPayload, Ptr<NetDevice> oif)
// ****** Mahdi Change ***** (END) ***** // 
{
    ...
    if (!allRoutes.empty()) // if route(s) is found
    {
        ...
        // ****** Mahdi Change ***** (START) ***** // 
        // pick one of the routes based on 5-tuple hash
        uint32_t selectIndex;
        if (m_randomEcmpRouting)
        {
            // ***** Mahdi Change ***** (START) ***** //
            // selectIndex = m_rand->GetInteger(0, allRoutes.size() - 1);
            selectIndex = ((GetHashValue(header, ipPayload)) % (allRoutes.size()));
            /* The corrupted ECMP: if we are on T0 Switch, route the packets that go through the Index 1 based on random uniform distribution*/
            // Ptr<Node> node = m_ipv4->GetObject<Node>();
            // if (node->GetId() == 24 && selectIndex == 0 && (header.GetSource() == "10.1.1.1" || header.GetSource() == "10.1.2.1"))
            // {
            //     uint32_t temp = m_rand->GetInteger(0, allRoutes.size() * 4 - 1);
            //     if (temp >= allRoutes.size() / 2)
            //     {
            //         selectIndex = 0;
            //     }
            //     else
                               ...
                               Socket::SocketErrno& sockerr)
{
    ...
    // ****** Mahdi Change ***** (START) ***** // 
    Ptr<Ipv4Route> rtentry = LookupGlobal(header.GetDestination(), header, p, oif);
    // ****** Mahdi Change ***** (END) ***** //
    if (rtentry)
    {
                              ...
                              const ErrorCallback& ecb)
{
    ...
    // ***** Mahdi Change ***** (START) ***** //
    Ptr<Ipv4Route> rtentry = LookupGlobal(header.GetDestination(), header, p);
    // ***** Mahdi Change ***** (END) ***** //
    if (rtentry)
    {

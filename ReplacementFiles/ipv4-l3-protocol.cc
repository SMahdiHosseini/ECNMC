...
// ***** Mahdi Change ***** (START) ***** //
uint64_t 
Ipv4L3Protocol::GetHashValue_out(const Ipv4Address src, const Ipv4Address dst, const uint16_t srcPort, const uint16_t dstPort, const uint8_t protocol)
{
    NS_LOG_FUNCTION(src << dst << srcPort << dstPort << protocol);
    m_hasher.clear();
    std::ostringstream oss;
    oss << src
        << dst
